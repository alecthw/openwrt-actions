...
local function select_node(nodes, config)
	...
	if config.currentNode then
		...
		if not server then
			for index, node in pairs(nodes) do
				if config.currentNode.add_from and config.currentNode.type and config.currentNode.remarks then
					if node.add_from and node.type and node.remarks then
						if node.add_from == config.currentNode.add_from and node.type == config.currentNode.type and node.remarks == config.currentNode.remarks then
							if config.log == nil or config.log == true then
								log('更新【' .. config.remarks .. '】自定义匹配节点：' .. node.remarks)
							end
							server = node[".name"]
							break
						end
