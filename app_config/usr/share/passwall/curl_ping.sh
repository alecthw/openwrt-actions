...
time_start_transfer=0 # 在发出请求之后，Web服务器返回数据的第一个字节所用的时间
...
for i in $(seq 1 ${count}); do
    result=$(curl -o /dev/null --connect-timeout 2 -k -s -w "%{http_code} %{time_connect} %{time_start_transfer} %{time_total}" https://${url})
    ...
    time_start_transfer=$(echo "${time_start_transfer} ${t_s_ms}" | awk '{printf ("%.3f\n",$1+$2)}')
    time_total=$(echo "${time_total} ${t_t_ms}" | awk '{printf ("%.3f\n",$1+$2)}')
done

ave_c=$(echo "${time_connect} ${count_c}" | awk '{printf ("%.3f\n",$1/$2)}')
ave_s=$(echo "${time_start_transfer} ${count_c}" | awk '{printf ("%.3f\n",$1/$2)}')
